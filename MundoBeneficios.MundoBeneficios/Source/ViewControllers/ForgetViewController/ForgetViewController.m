...
-(UIView*)getBodyView{
    ...
    [btnEnter addTarget:self action:@selector(recoveryPassword:) forControlEvents:UIControlEventTouchUpInside];
    return content;
}
...
-(IBAction)recoveryPassword:(id)sender{
    if (txtDNI.text.length>0) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(endPostRecoveryPassword:) name:@"endPostRecoveryPassword" object:nil];
        
        NSDictionary*params=@{PARAM_DOCUMENT:txtDNI.text};
        [[ProgressView sharedInstance] showInView:self.view withTitle:@"Enviando"];
        [[QueryWebService sharedInstance] getDataFromPath:FILE_RECOVERY_PASSWORD withMethod:POST withParamData:params withNotification:@"endPostRecoveryPassword"];
...
-(void)endPostRecoveryPassword:(NSNotification*)notification{
    [[ProgressView sharedInstance] hide];
    if ([[(NSDictionary*)notification.object objectForKey:@"statusCode"] intValue]==400 || [[(NSDictionary*)notification.object objectForKey:@"statusCode"] intValue]==404) {
        ...
        alertViewController.message = [(NSDictionary*)notification.object objectForKey:@"errorDescription"];
        // Add alert actions
        ...
        alertViewController.message = [(NSDictionary*)notification.object objectForKey:@"data"];
        // Add alert actions
        [alertViewController addAction:[NYAlertAction actionWithTitle:NSLocalizedString(@"TITLE_BTN_ACCEPT", nil)
                                                                style:UIAlertActionStyleCancel
                                                              handler:^(NYAlertAction *action) {
